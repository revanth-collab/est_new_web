// import { useEffect, useState } from "react"
// import { Link } from 'react-router-dom';

// const Login = () => {
//     const [number, setNumber] = useState('')
//     const [password, setPassword] = useState('')
//     const [errmsg, setErrMsg] = useState('')
//     const [error, setError] = useState(false);

//     const handleSubmit = (event) => {
//         event.preventDefault();
//         console.log(number);
//         console.log(password);
//     }

//     return (
//         <div className="min-h-screen flex flex-col md:flex-row justify-content items-center gap-5 m-3">
//             <div className="w-full md:w-3/5"></div>
//             <form className='w-full md:w-2/5 max-w-[400px] rounded-2xl p-10 shadow-md bg-white' onSubmit={handleSubmit}>
//                 <h1 className='font-semibold text-[24px]'>Login</h1>
//                 <p className='login-para font-medium text-[12px]'>Login to access your travelwise  account</p>
//                 <div className='flex flex-col gap-2 mt-4 w-full'>
//                     <label htmlFor='username' className='text-[#64748B] text-xs font-medium'>Mobile Number</label>
//                     <input
//                         type="number"
//                         id="number"
//                         value={number}
//                         placeholder="Enter the Number"
//                         className="p-2 border-[2px] border-[#D3DDEC] rounded-md outline-none"
//                         onChange={(e) => setNumber(e.target.value)}
//                     />
//                 </div>
//                 <div className='flex flex-col gap-2 mt-4 w-full'>
//                     <label htmlFor='password' className='text-[#64748B] text-xs font-medium'>Password</label>
//                     <input
//                         type="password"
//                         id="password"
//                         value={password}
//                         placeholder="Enter the Password"
//                         className="p-2 border-[2px] border-[#D3DDEC] rounded-md outline-none"
//                         onChange={(e) => setPassword(e.target.value)}
//                     />
//                 </div>
//                 <div className="w-full mt-5 flex flex-col justify-center items-center">
//                     <button className="w-[55%] btn text-white font-semibold px-auto py-3 rounded-xl mb-3" type="submit">Login</button>
//                     {/* <p className="text-[14px]">Don't have an account? <span className="primary-color font-bold cursor-pointer hover:underline">Sign Up</span></p> */}
//                     <p className="text-[14px]">
//                         Don't have an account?{" "}
//                         <Link
//                             to="/signup"
//                             className="primary-color font-bold cursor-pointer hover:underline underline-offset-2"
//                         >
//                             Sign Up
//                         </Link>
//                     </p>
//                 </div>
//             </form>
//         </div>
//     )
// }

// export default Login

import { useState } from "react";
import { Link, useNavigate } from "react-router-dom";

const Login = () => {
    const [phone, setPhone] = useState("");
    const [error, setError] = useState("");
    const navigate = useNavigate();

    const handleVerify = (e) => {
        e.preventDefault();

        if (!phone || phone.length !== 10) {
            setError("Enter a valid 10-digit mobile number.");
            return;
        }
    };

    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-white to-purple-100 px-4 py-10">
            <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 sm:p-10 text-center">
                <h1 className="text-3xl font-bold mb-2">Welcome!</h1>
                <p className="text-sm text-gray-600 mb-6">
                    Step into serenity with just a tap. Your journey to relaxation awaits.
                </p>

                <form onSubmit={handleVerify} className="space-y-4">
                    <div className="flex items-center border rounded-xl px-4 py-3 shadow-sm">
                        <span className="flex items-center gap-2 text-gray-700 font-medium text-sm">
                            🇮🇳 +91
                        </span>
                        <input
                            type="tel"
                            value={phone}
                            onChange={(e) => setPhone(e.target.value.replace(/\D/, ""))}
                            maxLength={10}
                            placeholder="Enter the Mobile Number"
                            className="ml-3 flex-1 bg-transparent focus:outline-none text-gray-800 text-sm"
                        />
                    </div>
                    {error && <p className="text-red-500 text-xs text-left">{error}</p>}

                    <p className="text-xs text-gray-500">
                        You’ll receive an OTP on the number above.
                    </p>

                    <p className="text-xs text-gray-500">
                        By login you agree to our{" "}
                        <Link to="/terms-and-condition"><span className="text-blue-500 underline cursor-pointer">Terms & Condition</span></Link> and{" "}
                        <Link to="/privacy-policy"><span className="text-blue-500 underline cursor-pointer">Privacy Policy</span></Link>.
                    </p>

                    <button
                        type="submit"
                        className="w-full bg-gradient-to-r from-purple-500 to-purple-400 hover:from-purple-600 hover:to-purple-500 text-white py-3 rounded-xl font-medium transition"
                        onClick={() => navigate('/otp', {
                            state: {
                                phone: phone
                            },
                        })}
                    >
                        Get Verification Code
                    </button>

                    <p className="text-sm mt-3">
                        Don’t have an account?{" "}
                        <Link
                            to="/signup"
                            className="text-purple-600 font-semibold hover:underline"
                        >
                            Sign up
                        </Link>
                    </p>
                </form>
            </div>
        </div>
    );
};

export default Login;

